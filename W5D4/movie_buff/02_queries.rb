...
def most_supportive
  ...
  Actor.select('actors.id, actors.name, COUNT(movies.id) AS roles').joins(:movies).where('castings.ord > 1').group('actors.id, actors.name').order('COUNT(movies.id) DESC').limit('2')
end
