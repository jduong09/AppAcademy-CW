...
def song_title_counts
  ...
  execute(<<-SQL)
    SELECT 
      tracks.song, COUNT(DISTINCT a.asin)
    FROM
      albums a
    JOIN
      tracks ON a.asin = tracks.album
    GROUP BY
      tracks.song
    HAVING
...
def best_value
  ...
  execute(<<-SQL)
    SELECT 
      albums.title, albums.price, COUNT(albums.asin) AS num_tracks
    FROM
      albums
    JOIN
      tracks on tracks.album = albums.asin
    ...
    HAVING
      albums.price/COUNT(albums.asin) < .50
    ;
  SQL
end
...
def top_track_counts
  ...
  execute(<<-SQL)
    SELECT
      albums.title, COUNT(albums.asin) AS num_tracks
    FROM
      albums
    JOIN
